...
class sparseKT(nn.Module):
    def __init__(self, num_skills, num_questions, seq_len, 
            embedding_size, num_blocks, dropout, kq_same, d_ff=256, 
            final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qr=False, emb_type="qid_sparseattn", sparse_ratio=0.8, k_index=5):
        super().__init__()
        """
        Input:
        ...
        self.model = Architecture(num_skills=num_skills, num_blocks=num_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    embedding_size=embedding_size, d_feature=embedding_size / num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, seq_len=seq_len)
    ...
    def forward(self, feed_dict):
        ...
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch"]:
            ...
            d_output, attn_weights = self.model(q_embed_data, qa_embed_data, emb_type, sparse_ratio, k_index)
            self.attn_weights = attn_weights
        ...
        pooled_ques_score = (self.q_embed(c) * attention_mask.unsqueeze(-1)).sum(
            1
        ...
        if self.training:
            chunk_size = 20  # 一次只处理100个skill
            
            B, T, E = d_output.shape
            K = self.num_skills
            ys = []
            for i in range(0, K, chunk_size):
                skill_idx = torch.arange(i, min(i + chunk_size, K), device=d_output.device)
                all_skills_emb = self.q_embed(skill_idx).unsqueeze(0).unsqueeze(0).repeat(B, T, 1, 1)
                h_4_all_skills = d_output.unsqueeze(2).repeat(1, 1, all_skills_emb.size(2), 1)
                concat_qs = torch.cat([h_4_all_skills, all_skills_emb], dim=-1)
                ys_chunk = self.out(concat_qs).squeeze(-1)
                ys.append(ys_chunk)
    
            ys = torch.cat(ys, dim=2)
            ys = torch.sigmoid(ys)[:, 1:, ...]
            out_dict = {
                "pred": preds[:, 1:],
                "true": r[:, 1:].float(),
                "ctf_result": ys,
...
class Architecture(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data, emb_type="qid", sparse_ratio=0.8, k_index=5):
        ...
        for block in self.blocks_2:
            x, attn_weights = block(mask=0, query=x, key=x, values=y, apply_pos=True, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index) # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, apply_pos=True, emb_type="qid", sparse_ratio=0.8, k_index=5):
        """
        Input:
        ...
        if mask == 0:  # If 0, zero-padding is needed.
            ...
            query2,_ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=True, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index) # 只能看到之前的信息，当前的信息也看不到，此时会把第一行score全置0，表示第一道题看不到历史的interaction信息，第一题attn之后，对应value全0
        else:
            # Calls block.masked_attn_head.forward() method
            query2,_ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=False, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad, emb_type="qid", sparse_ratio=0.8, k_index=5):
        ...
        scores, attn_weights = attention(q, k, v, self.d_k,
                           mask, self.dropout, zero_pad, emb_type, sparse_ratio=sparse_ratio, k_index=k_index)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, emb_type="qid", sparse_ratio=0.8, k_index=5):
    """
