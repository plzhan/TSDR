...
class DisKT(nn.Module):
    ...
    def forward(self, feed_dict):
        ...
        x = torch.cat([xh, q_embed_data], dim=-1)
        ...
        preds = m1(output)
        if self.training:
            # all_skills_emb = self.q_embed(torch.arange(0, self.num_skills))
            # h_4_all_skills = h.unsqueeze(2).repeat(1, 1, self.num_skills, 1)
            # y_diff_4_all_skills = (y1-y2).unsqueeze(2).repeat(1, 1, self.num_skills, 1)
            # concat_qs = torch.cat([h_4_all_skills, all_skills_emb], dim=-1)
            # concat_qs = torch.cat([concat_qs, y_diff_4_all_skills], dim=-1)
            # ys = self.out(concat_qs).squeeze(-1)
            # ys = nn.Sigmoid()(ys)[:, 1:, ...]
            
            B, T, E = xh.shape
            K = self.num_skills
            ys = []
            chunk_size = 50  # 一次只处理100个skill
            for i in range(0, K, chunk_size):
                skill_idx = torch.arange(i, min(i + chunk_size, K), device=xh.device)
                all_skills_emb = self.q_embed(skill_idx).unsqueeze(0).unsqueeze(0).repeat(B, T, 1, 1)
                h_4_all_skills = xh.unsqueeze(2).repeat(1, 1, all_skills_emb.size(2), 1)
                #print("h_4_all_skills shape", h_4_all_skills.shape)
                concat_qs = torch.cat([h_4_all_skills, all_skills_emb], dim=-1)
                #print("y1 - y2 shape", (y1 - y2).shape)
                concat_qs = torch.cat([concat_qs, (y1 - y2).unsqueeze(2).repeat(1, 1, all_skills_emb.size(2), 1)], dim=-1)
