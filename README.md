...
```powershell
cd TemporalSmoothnessDoublyRobustLearningforDebiasedKnowledgeTracing
python main.py --model_name akt --data_name prob
```

启用 TSDR 风格的 doubly robust 训练：

```powershell
python main.py --model_name akt --data_name prob --dr --hp_lambda 0.3
```

论文实验中可以直接按下面的形式运行 `AKT + prob + DR`：
